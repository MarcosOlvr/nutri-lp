                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 lg:gap-6">
                    {plans.map((plan, key) => (
                        <div key={key} className={`relative bg-silver shadow-2xl backdrop-blur-sm border rounded-xl sm:rounded-2xl p-6 sm:p-8 transition-all duration-300 overflow-visible group flex flex-col h-full ${
                            ...
                            : "hover:border-white hover:scale-101"
                            }`}>
                                ...
                                <div className="absolute -top-2 sm:-top-3 left-1/2 transform -translate-x-1/2 z-10">
                                    <div className="flex items-center space-x-1 px-3 sm:px-4 py-1 sm:py-1.5 bg-green rounded-full text-xs sm:text-sm font-semibold shadow-lg">
                            ...
                            <div className="text-center mb-6 sm:mb-8">
                                <h3 className="text-xl font-bold mb-2 text-preto">{plan.name}</h3>
                                <p className="text-d-grey text-xs sm:text-sm mb-3 sm:mb-4">{plan.desc}</p>
                                <div className="flex items-baseline justify-center">
                                    <span className="text-3xl sm:text-4xl lg:text-5xl font-bold text-green">R${plan.price}</span>
                                </div>
                                <span className="text-emerald-950 ml-1 sm:ml-2 text-sm sm:text-base">{plan.pricePerMonth}/mês</span>
                            </div>
                            ...
                            <ul className="space-y-3 sm:space-y-4 mb-6 sm:mb-8 flex-row">
                                {plan.features.map((feature, featureKey) => (
                                  <li key={featureKey} className="flex items-start space-x-2 sm:space-x-3">
                                    <div className="flex-shrink-0 w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-emerald-100 flex items-center justify-center mt-0.5">
                                        <Check className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-green"/>
                                    </div>
                                    <span className="text-d-grey text-sm">{feature}</span>
                                  </li>
                                ))}
                            </ul>
