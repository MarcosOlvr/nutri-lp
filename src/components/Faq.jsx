      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row gap-10">
          ...
          <div className="flex flex-col gap-3 flex-1">
            {faqs.map((faq, index) => (
              <div key={index} className="border-1 border-black rounded-xl overflow-hidden hover:bg-white/85 transition-colors">
                <button
                  onClick={() => setOpen(open === index ? null : index)}
                  className={`w-full flex justify-between items-center px-6 py-4 text-left font-medium transition-colors ${open === index ? "text-green" : "text-black"}`}
                >
                ...
                {open === index && (
                  <div className="px-6 py-4 text-d-grey text-base leading-relaxed border-t border-black">
                    {faq.answer}
                  </div>
                )}
              </div>
